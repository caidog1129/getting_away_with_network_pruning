...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_points', type=int, default=9)
    ...
    for seed in range(times):
        ...
        print("LR IS,", lr)
        print("TIME IS,", after_time-current_time)
    ...
    for p in tqdm(sp):
        ...
        # compressions = [[1/ast.literal_eval(sparsity[np.argmax(val)])[0], 1/ast.literal_eval(sparsity[np.argmax(val)])[1]]]
...
#     new.append(sum_a)
        ...
        # new_lr.append(lr_lst[0])
        # new.append(acc_lst[0])
    ...
    df.to_csv(f'{model_arch}_{method}_{sample_points}_{dataset}_updated.csv')
    print('got the csv')
    df = pd.DataFrame(log)
    df.to_csv(f'{model_arch}_{method}_{sample_points}_{dataset}_acc_updated.csv')